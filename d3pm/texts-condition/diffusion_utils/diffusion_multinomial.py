...
class MultinomialDiffusion(torch.nn.Module):
    ...
    @torch.no_grad()
    def p_sample(self, log_x, c, t):
        model_log_prob = self.p_pred(log_x=log_x, c=c, t=t)
    ...
    @torch.no_grad()
    def _sample(self, c, image_size, batch_size = 16):
        return self.p_sample_loop(c, (batch_size, 3, image_size, image_size))

    @torch.no_grad()
    def interpolate(self, x1, x2, c, t = None, lam = 0.5):
    ...
    def compute_Lt(self, x_start, x_t, c, t, detach_mean=False):
        true_logits = self.q_posterior(x_start=x_start, x_t=x_t, t=t, x_start_logits=False)
        model_logits, pred_x_start_logits = self.p_pred(x=x_t, c=c, t=t)
    ...
    def sample(self, c, num_samples):
        ...
        for i in reversed(range(0, self.num_timesteps)):
            ...
            log_z = self.p_sample(log_z, c, t)
        print()
        return log_onehot_to_index(log_z)

    def sample_chain(self, c, num_samples):
        b = num_samples
        device = self.log_alpha.device
